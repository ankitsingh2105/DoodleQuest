...
export default function Dashboard() {
  ...
  useEffect(()=>{
    const fetchFollowers = async () =>{
      try{
        const res = await axios.get(`${backendLink}/users/followers/${userName}`, { withCredentials: true });
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {followers.map((follower) => (
                <tr key={follower.follower_userName} className="hover:bg-pink-50">
                  <td className="p-2 border-b border-pink-200">{follower.follower_userName}</td>
                  <td className="p-2 text-pink-500 font-bold border-b border-pink-200"><a href={`https://doodlequest.games/${follower.follower_userName}`}>Link to {`${follower.follower_userName}'s profile`} </a></td>
